...
exports.confirmarCompra = async (req, res) => {
  try {
    ...
    for (const persona of personasResult.rows) {
      ...
      const qrData = getTicketUrl(uuidEntrada);
      ...
      personasConQr.push({
        nombre_completo: persona.nombre_completo,
        qr_url: getQrUrl(uuidEntrada),
        ticket_url: getTicketUrl(uuidEntrada),
        apple_wallet_url: getAppleWalletUrl(uuidEntrada),
        google_wallet_url: generarGoogleWalletUrl(entradaWallet)
      });
...
exports.obtenerAppleWallet = async (req, res) => {
  try {
    ...
    const passBuffer = await generarApplePass(entrada);

    res.setHeader('Content-Type', 'application/vnd.apple.pkpass');
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="zairo-${entrada.uuid_entrada}.pkpass"`
    );

    return res.send(passBuffer);

  } catch (error) {
    console.error('ERROR GENERANDO APPLE WALLET:', error);
